...
// (rename drift). Advanced Metadata Cache's feature is an extended `getBacklinksForFile`
// API with no dedicated public interface type, so only the PluginSettings config
// class is reflected; the runtime behavior is covered by the other integration tests.
registerDemoVaultCoverageSuite({
  ...
  nonTrivialGuard: {
    ...
    expectMember: 'shouldShowProgressBarOnLoad',
