 ...
 * changed name, which makes a bulk folder delete O(N²). With `advanced-metadata-cache`
...
/*
 * Deadline for Obsidian's startup scan and advanced-metadata-cache's initial processAllNotes to
...
describe('updateRelatedLinks avoids the vault scan', () => {
  it('queues only the linking files and never calls getCachedFiles', async () => {
    ...
    // Advanced-metadata-cache indexed every note that links to the target.
